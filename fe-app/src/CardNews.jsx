...
export default function CardNews({article,myNews}){
    ...
    return(
        ...
        <div className="div-card-news">
            ...
            <div className="read-more"><Link to={`/details/${article?.title}`}>Read more...</Link>{myNews ==1 && <><button>Delete</button><Link to={`/editnews/${article?.title}`}><button>Edit</button></Link></>}</div>
